...
def test_protect_front_matter_multiline_field_accepts_tab_indented_body_lines() -> None:
    ...
    assert block.block_type == "front_matter"
    assert block.translatable is True
    assert "鍒涘缓鏂囨。   " in block.protected_text
    assert "@@PROTECT_0001@@" in block.protected_text
    assert "author: 寮犱笁" not in block.protected_text

    translated_texts = [
        "---\n"
        "@@PROTECT_0000@@\n"
        "@@PROTECT_0001@@Create document.   \n"
        "@@PROTECT_0002@@\n"
        "---"
    ]
...
def test_protect_code_block_with_python_comments_extracts_comments_for_translation() -> None:
    text = (
        "```python\n"
        "# 这是一个示例函数\n"
        "def hello():\n"
    ...
    assert block.translatable is True
    assert "@@PROTECT_" in block.protected_text
    # Comment content should be exposed (not in placeholders)
    assert "这是一个示例函数" in block.protected_text
    assert "打印问候语" in block.protected_text
    # Code should be protected
    assert "def hello():" not in block.protected_text
    assert "print('hello')" not in block.protected_text

    restored = protector.restore_document(protected)
    assert restored == text


def test_protect_code_block_with_c_comments_extracts_comments_for_translation() -> None:
    text = (
        "```c\n"
        "// 这是一个示例函数\n"
        "void hello() {\n"
    ...
    assert block.block_type == "fenced_code_block"
    assert block.translatable is True
    # Comment content should be exposed
    assert "这是一个示例函数" in block.protected_text
    assert "打印问候语" in block.protected_text
    assert "多行" in block.protected_text
    assert "注释" in block.protected_text

    restored = protector.restore_document(protected)
    assert restored == text


def test_protect_code_block_without_language_is_not_translatable() -> None:
    text = (
        "```\n"
        "# 这是一个注释\n"
        "echo hello\n"
    ...
    assert block.block_type == "fenced_code_block"
    assert block.translatable is False
    # Entire block should be a single placeholder
    assert block.protected_text.startswith("@@PROTECT_")
...
def test_protect_code_block_without_comments_is_not_translatable() -> None:
    text = (
        "```python\n"
        "def hello():\n"
        "    print('hello')\n"
...
def test_translate_code_block_comments_and_restore() -> None:
    """Test full translation flow for code block comments."""
    ...
    original_segment = segmented.segments[0].text
    translated_segment = original_segment.replace("这是一个示例", "This is a sample")

    translated_texts = {"seg-0000": translated_segment}
    rebuilt_blocks = segmenter.rebuild_protected_block_texts(segmented, translated_texts)
