...
def test_dita_handler_preserves_in_tree_comments_processing_instructions_and_prefixes(
    tmp_path: Path,
) -> None:
    handler = DitaDocumentHandler()
    source_text = (
        "<topic xmlns:x=\"urn:test\" id=\"demo\">"
        "<!-- keep -->"
        "<title>标题</title>"
        "<?work step?>"
        "<body><p x:role=\"ui\">内容</p></body>"
        "</topic>\n"
    )
    ...
    rebuilt = handler.rebuild_document(
        ...
        {
            prepared.segments[0].id: "Title",
            prepared.segments[1].id: "Body",
        },
    )

    assert "<!-- keep -->" in rebuilt
    assert "<?work step?>" in rebuilt
    assert 'xmlns:x="urn:test"' in rebuilt
    assert 'x:role="ui"' in rebuilt
    assert "ns0:role" not in rebuilt
    assert "<title>Title</title>" in rebuilt
    assert ">Body</p>" in rebuilt
